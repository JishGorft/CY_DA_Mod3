...
SELECT DAYNAME(order_datetime) AS dow, COUNT(*) AS orders_count FROM orders
	WHERE status = 'paid'
	GROUP BY dow
...
SELECT DATE(order_datetime) AS order_date, COUNT(*) AS orders_count FROM orders
	GROUP BY DATE(order_datetime)
	HAVING COUNT(*) > 3; 
-- Dumb example? Nothing is returned, but the code works (test by changing HAVING COUNT(*) > 1;)
...
SELECT (100.0 * SUM(CASE WHEN payment_method = 'app' THEN 1 ELSE 0 END) / COUNT(*)) AS pct_app_paid_orders 
FROM orders
WHERE status = 'paid';
-- Had to lean on W3 Schools for this one. Not 100% sure CASE was the only way to do this?
